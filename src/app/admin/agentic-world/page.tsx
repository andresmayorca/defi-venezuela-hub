...
const empty = { title: "", description: "", difficulty: "Intermedio", icon: "🤖", tools: "", published: false };

export default function AdminAgenticPage() {
  const [items, setItems] = useState<AgenticProject[]>([]);
  ...
  useEffect(() => { load(); }, []);
  async function load() {
    const { data } = await supabase.from("agentic_projects").select("*").order("created_at", { ascending: false });
    setItems(data ?? []);
  }

  async function save() {
    setSaving(true);
    const payload = { ...form, tools: form.tools.split(",").map((t) => t.trim()).filter(Boolean) };
    if (editId) { await supabase.from("agentic_projects").update(payload).eq("id", editId); }
    else { await supabase.from("agentic_projects").insert(payload); }
    await load(); setSaving(false); setModal(false); setForm(empty); setEditId(null);
  }

  async function toggle(id: string, published: boolean) { await supabase.from("agentic_projects").update({ published }).eq("id", id); await load(); }
  async function del(id: string) { if (!confirm("¿Eliminar?")) return; await supabase.from("agentic_projects").delete().eq("id", id); await load(); }
  async function notify(item: AgenticProject) {
    if (!confirm(`¿Notificar sobre "${item.title}"?`)) return;
    await fetch("/api/notify", { method: "POST", headers: { "Content-Type": "application/json", "x-admin-secret": process.env.NEXT_PUBLIC_ADMIN_SECRET ?? "" }, body: JSON.stringify({ section: "agentic", title: item.title, url: "/agentic-world" }) });
    alert("Notificaciones enviadas ✓");
  }

  function openEdit(item: AgenticProject) {
    setForm({ title: item.title, description: item.description, difficulty: item.difficulty, icon: item.icon, tools: (item.tools ?? []).join(", "), published: item.published });
    setEditId(item.id); setModal(true);
  }

  const difficultyOptions = ["Principiante", "Intermedio", "Avanzado"];
  const diffColors: Record<string, string> = { Principiante: "text-green-400", Intermedio: "text-[#00FF88]", Avanzado: "text-orange-400" };

  return (
    <div className="p-8">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold text-[#F0F1F5]">Agentic World</h1>
        <button onClick={() => { setForm(empty); setEditId(null); setModal(true); }} className="flex items-center gap-2 px-4 py-2 rounded-xl bg-[#00FF88] text-[#0D1117] font-semibold text-sm hover:bg-[#00e67a] transition-colors">+ Nuevo proyecto</button>
      </div>

      {items.length === 0 ? (
        <div className="text-center py-16 text-[#9DA5B4]/40"><p className="text-4xl mb-3">🤖</p><p>No hay proyectos.</p></div>
      ) : (
        <div className="rounded-xl border border-white/5 overflow-hidden">
          <table className="w-full text-sm">
            <thead><tr className="bg-[#161B24] border-b border-white/5">
              <th className="text-left px-4 py-3 text-[#9DA5B4]/60 font-semibold text-xs uppercase tracking-wider">Proyecto</th>
              <th className="text-left px-4 py-3 text-[#9DA5B4]/60 font-semibold text-xs uppercase tracking-wider">Dificultad</th>
            ...
            <tbody>
              {items.map((item) => (
                <tr key={item.id} className="border-b border-white/5 hover:bg-white/2 transition-colors">
                  <td className="px-4 py-3 text-[#F0F1F5] font-medium">{item.icon} {item.title}</td>
                  <td className="px-4 py-3"><span className={`text-xs font-medium ${diffColors[item.difficulty] ?? "text-[#9DA5B4]"}`}>{item.difficulty}</span></td>
        ...
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
          <div className="w-full max-w-lg rounded-2xl border border-white/10 bg-[#161B24] p-6 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-5">
              <h2 className="font-bold text-[#F0F1F5]">{editId ? "Editar proyecto" : "Nuevo proyecto"}</h2>
              <button onClick={() => setModal(false)} className="text-[#9DA5B4]/60 hover:text-[#F0F1F5]"><X className="w-5 h-5" /></button>
            </div>
            <div className="flex flex-col gap-4">
              {[{ key: "icon", label: "Emoji" }, { key: "title", label: "Título" }, { key: "tools", label: "Tools (separadas por coma)" }].map((f) => (
                <div key={f.key}>
              ...
              <div>
                <label className="block text-xs text-[#9DA5B4]/60 font-semibold mb-1.5 uppercase tracking-wider">Dificultad</label>
