...
const empty = { title: "", emoji: "📅", date: "", time: "", location: "", type: "Meetup", tags: "", description: "", link: "", featured: false, published: false };
...
export default function AdminEventosPage() {
  ...
  async function save() {
    ...
    if (editId) { await supabase.from("eventos").update(payload).eq("id", editId); }
    else { await supabase.from("eventos").insert(payload); }
    await load(); setSaving(false); setModal(false); setForm(empty); setEditId(null);
  }

  async function toggle(id: string, published: boolean) { await supabase.from("eventos").update({ published }).eq("id", id); await load(); }
  async function del(id: string) { if (!confirm("¿Eliminar?")) return; await supabase.from("eventos").delete().eq("id", id); await load(); }
  async function notify(item: Evento) {
    if (!confirm(`¿Notificar sobre "${item.title}"?`)) return;
    await fetch("/api/notify", { method: "POST", headers: { "Content-Type": "application/json", "x-admin-secret": process.env.NEXT_PUBLIC_ADMIN_SECRET ?? "" }, body: JSON.stringify({ section: "eventos", title: item.title, url: "/eventos" }) });
    alert("Notificaciones enviadas ✓");
  }

  function openEdit(item: Evento) {
    setForm({ title: item.title, emoji: item.emoji, date: item.date, time: item.time, location: item.location, type: item.type, tags: "", description: item.description, link: item.link, featured: item.featured, published: item.published });
    setEditId(item.id); setModal(true);
  }

  const typeOptions = ["Conferencia", "Hackathon", "Workshop", "Meetup", "AMA", "Webinar"];

  return (
    <div className="p-8">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold text-[#F0F1F5]">Eventos</h1>
        <button onClick={() => { setForm(empty); setEditId(null); setModal(true); }} className="flex items-center gap-2 px-4 py-2 rounded-xl bg-[#00FF88] text-[#0D1117] font-semibold text-sm hover:bg-[#00e67a] transition-colors">+ Nuevo evento</button>
      </div>

      {items.length === 0 ? (
        <div className="text-center py-16 text-[#9DA5B4]/40"><p className="text-4xl mb-3">📅</p><p>No hay eventos.</p></div>
      ) : (
        <div className="rounded-xl border border-white/5 overflow-hidden">
          <table className="w-full text-sm">
            <thead><tr className="bg-[#161B24] border-b border-white/5">
              <th className="text-left px-4 py-3 text-[#9DA5B4]/60 font-semibold text-xs uppercase tracking-wider">Evento</th>
              <th className="text-left px-4 py-3 text-[#9DA5B4]/60 font-semibold text-xs uppercase tracking-wider">Fecha</th>
              <th className="text-left px-4 py-3 text-[#9DA5B4]/60 font-semibold text-xs uppercase tracking-wider">Tipo</th>
            ...
            <tbody>
              {items.map((item) => (
                <tr key={item.id} className="border-b border-white/5 hover:bg-white/2 transition-colors">
                  ...
                  <td className="px-4 py-3 text-[#9DA5B4] text-xs">{item.date}</td>
                  <td className="px-4 py-3"><span className="text-xs px-2 py-0.5 rounded-full bg-purple-400/10 text-purple-400">{item.type}</span></td>
        ...
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
          <div className="w-full max-w-lg rounded-2xl border border-white/10 bg-[#161B24] p-6 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-5">
              <h2 className="font-bold text-[#F0F1F5]">{editId ? "Editar evento" : "Nuevo evento"}</h2>
            ...
            <div className="flex flex-col gap-4">
              {[
                ...
                { key: "date", label: "Fecha (ej: 15 Junio 2026)" },
                { key: "time", label: "Hora (ej: 3:00 PM VET)" },
                { key: "location", label: "Lugar" },
                { key: "link", label: "URL registro" },
              ...
              <div>
                <label className="block text-xs text-[#9DA5B4]/60 font-semibold mb-1.5 uppercase tracking-wider">Tipo</label>
